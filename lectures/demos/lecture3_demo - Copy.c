...
/* 
 * Filename: Lecture_3_Demo.c
 *
 * Description: Demo size and sign conversions in C
 *              as well as their impact. 
 *
 * Auhtor: AL and the authors of our textbook
...
int main(int argc, char *argv[]) {
  ...
  int x = 12;
  int y = 2;
  printf("Shifting (let's use w = 8 for sake of space efficiency)\n");
  printf("Let's shift x (%d) left by y (%d): x << y = %d\n", x, y, x << y); 
  int xi = -12;
  printf("Let's shift xi (%d) right by y (%d) : xi >> y = %d\n", xi, y, xi >> y); 
  unsigned int xu = 12;
  printf("Let's shift xu (%d) right by y (%d) : xu >> y = %d\n", xu, y, xu >> y); 
  ...
  int anInt = aShort;
  printf("As an int data type, the variable anInt has the value %d (hex: 0x", anInt);
